...
def build_mosaic(result_queue, all_tile_data_large, original_img_large, out_file):
...
def compose(original_img, tiles, out_file):
	...
	try:
		Process(target=build_mosaic, args=(result_queue, all_tile_data_large, original_img_large, out_file)).start()
...
def mosaic(img_path, tiles_path, out_file = OUT_FILE):
	tiles_data = TileProcessor(tiles_path).get_tiles()
	image_data = TargetImage(img_path).get_data()
	compose(image_data, tiles_data, out_file)

if __name__ == '__main__':
	if len(sys.argv) < 3:
		print('Usage: {} <image> <tiles directory> <output directory> \r'.format(sys.argv[0]))
	elif len(sys.argv) == 3:
